...
public class OfflineSyncManager implements FormatListener {
    ...
    public static boolean isTwoFactorAuthSetupError(Exception exception) {
        if (exception instanceof ServiceException) {
            return ((ServiceException)exception).getCode().equals(AccountServiceException.TWO_FACTOR_SETUP_REQUIRED);
        }
        return false;
    }

    public static boolean isTwoFactorAuthRequiredError(Exception exception) {
        if (exception instanceof ServiceException) {
            return ((ServiceException)exception).getCode().equals(AccountServiceException.TWO_FACTOR_AUTH_REQUIRED);
        }
        return false;
    }

    public static boolean isTwoFactorAuthExpiredError(Exception exception) {
        if (exception instanceof ServiceException) {
            return ((ServiceException)exception).getCode().equals(AccountServiceException.TWO_FACTOR_AUTH_EXPIRED);
        }
        return false;
    }

    public static boolean isTwoFactorAuthFailedError(Exception exception) {
        if (exception instanceof ServiceException) {
            return ((ServiceException)exception).getCode().equals(AccountServiceException.TWO_FACTOR_AUTH_FAILED);
        }
        return false;
    }
    ...
    private void processSyncException(NamedEntry entry, String password, Exception exception, boolean isDebugTraceOn, boolean markSyncFail) {
        ...
        } else if (isTwoFactorAuthSetupError(exception) || isTwoFactorAuthRequiredError(exception)
                || isTwoFactorAuthExpiredError(exception) || isTwoFactorAuthFailedError(exception)) {
            authFailed(entry, code, password);
            if (isDebugTraceOn) {
                OfflineLog.offline.debug("sync two factor error: " + entry.getName(), exception);
            } else {
                OfflineLog.offline.warn("sync two factor error: " + entry.getName(), exception);
            }
