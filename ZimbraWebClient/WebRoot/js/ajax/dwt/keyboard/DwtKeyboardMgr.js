...
//	DBG.println("kbnav", "PUSH default handler: " + handler);
	...
	if (this.__currDefaultHandler &&
			this.__currDefaultHandler._zIndex && handler._zIndex &&
			(this.__currDefaultHandler._zIndex > handler._zIndex)) {
		/* If the new handler has z-index lower than that of the __curDefaultHandler,
		 * then we place it below the __curDefaultHandler.
		 */
		this.__defaultHandlerStack.splice((this.__defaultHandlerStack.length-1), 0, handler);
	}
	else {
