...
public class LuceneViewer {
     ...
     * that matched any filter. If filters are not used, null.
     */
    ...
    public LuceneViewer(String indexName, String outfileName,
            TermFilters termFilters, Console console) throws Exception {
        mIndexDir = indexName;
    ...
    private void dumpTermFilters() throws IOException {
        ...
        for (TermFilters.TermFilter termFilter : getFilters()) {
            String field = termFilter.mField;
            String text = termFilter.mText;
            outputLn("   (field: " + (field == null ? "" : field) +
                    ") (text: " + (text == null ? "" : text) + ")");
        }
    }
    ...
    private static class CLI {
        ...
        private Options getTermFilterOptions() {
            ...
            for (int i = 1; i <= NUM_TERM_FILTERS; i++) {
                options.addOption(termFilterFieldOption(i), "field" + i, true,
                        "field name of term filter " + i);
                options.addOption(termFilterTextOption(i), "text" + i, true,
                        "text of term filter " + i);
            }
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (!cl.hasOption(CLI.O_ACTION)) {
            cli.usage(new ParseException(
                    "missing required option " + CLI.O_ACTION), true);
        }
