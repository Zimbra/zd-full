...
public class OfflineProvisioning extends Provisioning implements OfflineConstants {
    ...
    public static final String A_offlineAutoArchiveEnabled = "zimbraPrefAutoArchiveEnabled";
    public static final String A_offlineAutoArchiveNoOfDays = "zimbraPrefAutoArchiveNoOfDays";
    public static final String A_offlineAutoArchiveFrequency = "zimbraPrefAutoArchiveFrequency";

    public static final String A_offlinePasswordLockEnabled = "zimbraPrefPasswordLockEnabled";
    ...
    public ZMailbox newZMailbox(OfflineAccount account, String serviceUri) throws ServiceException {
        ...
        if (authToken != null) {
            ...
            options = new ZMailbox.Options(account.getAttr(Provisioning.A_mail), AccountBy.name, account.getAttr(A_offlineRemotePassword), uri,  account.getAttr(A_zimbraTwoFactorAuthTrustedDeviceTokenKey));
        }
    ...
    private ZMailbox newZMailbox(String email, String password, Map<String, Object> attrs, String serviceUri) throws ServiceException {
        ...
        ZMailbox.Options options = new ZMailbox.Options(email, AccountBy.name, password, uri,  (String)attrs.get(A_zimbraTwoFactorAuthTrustedDeviceTokenKey));
        Object val = attrs.get(OfflineConstants.A_twofactorAuthCode);
        if (val instanceof String) {
            options.setTwoFactorCode((String) val);
        }
...
//        OfflineSyncManager.getInstance().authSuccess(options.getAccount(), options.getPassword(), zmbox.getAuthResult().getAuthToken(), zmbox.getAuthResult().getExpires());
    ...
    synchronized void modifyAttrs(Entry e, Map<String, ? extends Object> attrs, boolean checkImmutable, boolean allowCallback, boolean markChanged, boolean skipAttrMgr) throws ServiceException {
        ...
        if (isAccountSetup && etype == EntryType.ACCOUNT) {
            try {
                newAttributes = revalidateRemoteLogin((OfflineAccount) e, attrs);
                ...
                if (OfflineSyncManager.isTwoFactorAuthSetupError(exception) ||
                    ...
                    /* There is a chance that we were re-validating password and in response we got one of
                     * these two factor related exceptions.This implies that the provided password
                     * was correct and we should save it and update the error code.
                     */

                    // clear the old error code. Next sync request will set the appropriate one.
                    OfflineSyncManager.getInstance().clearErrorCode((OfflineAccount) e);

                    // Because we have got two factor error remove the two
                    // factor code if any and then save the remaining attributes.
                    attrs.remove(A_twofactorAuthCode);
                    DbOfflineDirectory.modifyDirectoryEntry(etype, A_zimbraId, e.getAttr(A_zimbraId), attrs,
                            markChanged);

                    reload(e);
                    DirectorySync.getInstance().sync((OfflineAccount) e, true);

                } else if (attrs.containsKey(A_twofactorAuthCode) // We were trying to set two factor code
                        && OfflineSyncManager.isAuthError(exception)) { // but Auth failed due to incorrect password.
        ...
        if (etype == EntryType.CONFIG) {
            ...
            if (newAttributes != null) {
                DbOfflineDirectory.modifyDirectoryEntry(etype, A_zimbraId, e.getAttr(A_zimbraId), newAttributes, markChanged);
            }
    ...
    private Map<String, Object> revalidateRemoteLogin(OfflineAccount acct, Map<String,? extends Object> changes) throws ServiceException {
        ...
        for (Map.Entry<String, ? extends Object> change : changes.entrySet()) {
            ...
            if (name.equalsIgnoreCase(A_offlineRemotePassword)) {
                ...
                baseUri = newBaseUri;
            } else if (name.equalsIgnoreCase(A_twofactorAuthCode)) { //Two factor code is changed. We need to update it.
                twoFactorCode = (String)change.getValue();
            }
        }
        ...
        // fetch the mailbox; this will throw an exception if the
        // username/password/URI are incorrect
        ZMailbox.Options options = new ZMailbox.Options(acct.getAttr(Provisioning.A_mail), AccountBy.name, password,
                Offline.getServerURI(baseUri, AccountConstants.USER_SERVICE_URI),
                acct.getAttr(A_zimbraTwoFactorAuthTrustedDeviceTokenKey));
        ...
        ZMailbox zm = newZMailbox(options);
        Map<String, Object> result = null;

        // If we had new two factor then, we might have new trusted token.
        if (twoFactorCode != null) {
            String trustedToken = zm.getAuthResult().getTrustedToken();
            if (trustedToken != null) {
                result = new HashMap<String, Object>();
                result.put(A_zimbraTwoFactorAuthTrustedDeviceTokenKey, trustedToken);
            }
        }
    ...
    static final Set<String> sOfflineAttributes = new HashSet<String>(Arrays.asList(
            ...
            A_offlineAutoArchiveEnabled,
            A_offlineAutoArchiveNoOfDays,
            A_offlineAutoArchiveFrequency,
            A_offlinePasswordLockEnabled
    ));
    ...
    public synchronized Account createAccount(String emailAddress, String password, Map<String, Object> attrs) throws ServiceException {
        ...
        if (dsName != null) {
            ...
            //Don't allow to add ZCS account, if auto archiving is already configured for ZCS account.
            if (AutoArchive.isAutoArchivingEnabled() && getAllZcsAccounts().size() != 0) {
                throw ServiceException.MULTIPLE_ZCS_ACCOUNT_NOT_SUPPORTED_AUTO_ARCHIVING_ENABLED();
            }
    ...
    private synchronized Account createSyncAccount(String emailAddress, String password, Map<String, Object> attrs) throws ServiceException {
        ...
        ZMailbox zm = newZMailbox(emailAddress, (String)attrs.get(A_offlineRemotePassword), attrs, AccountConstants.USER_SERVICE_URI);
        ZGetInfoResult zgi = zm.getAccountInfo(false);
        ...
        String trustedToken = zm.getAuthResult().getTrustedToken();
        if (trustedToken != null) {
            attrs.put(A_zimbraTwoFactorAuthTrustedDeviceTokenKey, trustedToken);
        }
    ...
    private synchronized Account createLocalAccount() throws ServiceException {
        ...
        attrs.put(A_offlineAutoArchiveEnabled, FALSE);
        attrs.put(A_offlinePasswordLockEnabled, FALSE);
        setDefaultAccountAttributes(attrs);
    ...
    private synchronized Account createAccountInternal(String emailAddress, String accountId, Map<String, Object> attrs, boolean initMailbox, boolean skipAttrMgr) throws ServiceException {
        ...
        if (!skipAttrMgr) {
            ...
            try {
                AttributeManager.getInstance().preModify(validAttrs, null, context, true, true);
                attrs = validAttrs;
            } catch (ServiceException e) {
    ...
    public void deleteAccount(String zimbraId) throws ServiceException {
        ...
        List<Account> zcsAccounts = getAllZcsAccounts();
        if (localAccount.isFeatureNotebookEnabled() && zcsAccounts.size() == 0) {
            localAccount.setFeatureNotebookEnabled(false);
        }

        //Reset auto archive preferences.
        if (zcsAccounts.size() == 0) {
            resetAutoArchivingPrefs();
        }
    }

    /**
     * Reset auto archiving preferences.
     * @throws ServiceException
     */
    private void resetAutoArchivingPrefs() throws ServiceException {
        Account localAccount = getLocalAccount();
        Map<String,Object> attrs = new HashMap<String,Object>();
        attrs.put(A_offlineAutoArchiveEnabled, null);
        attrs.put(A_offlineAutoArchiveNoOfDays, null);
        attrs.put(A_offlineAutoArchiveFrequency, null);
        attrs.put(A_offlineLastAutoArchive, null);
        modifyAttrs(localAccount, attrs);
    }
    ...
    void modifyDataSource(Account account, String dataSourceId, Map<String, Object> attrs, boolean markChanged) throws ServiceException {
        ...
        if (ds.getBooleanAttr(A_zimbraDataSourceContactSyncEnabled, false)) {
            if (ds instanceof OfflineDataSource && ((OfflineDataSource)ds).isYahoo()) {
                //TODO: implement new logic for Yahoo! contact sync. Yahoo! stopped mail API support.
            }
        }
    }
    ...
    public OfflineDomainGal createDomainGal(String domain, Map<String, Object> attrs) throws ServiceException {
        ...
        attrs.put(A_offlineGalRetryEnabled, TRUE);
