...
function() {
	var sections = {
		GENERAL: {
			...
			prefs: [
                ...
                ZmSetting.PASSWORD_LOCK_ENABLED
			]
		},
		COMPOSING: {
			parentId: "MAIL",
	...
	if (appCtxt.isOffline && appCtxt.accountList.accountTypeExists(ZmAccount.TYPE_ZIMBRA)) {
		sections["AUTO_ARCHIVING"] = {
			title: ZmMsg.autoArchiveTitle,
			icon: "AutoArchive",
			templateId: "prefs.Pages#AutoArchive",
			priority: 30,
			prefs: [
			     ZmSetting.AUTO_ARCHIVE_ENABLED,
			     ZmSetting.AUTO_ARCHIVE_AGE,
			     ZmSetting.AUTO_ARCHIVE_FREQ
			],
			createView: function(parent, section, controller) {
				return new ZmAutoArchivePage(parent, section, controller);
			}
		}
	}
    ...
    var autoArchiveFreqOptions = [ZmMsg.autoArchiveOnAppLaunch, ZmMsg.daily, ZmMsg.weekly, ZmMsg.monthly];
    var autoArchiveFreqValueOptions = ["on_app_launch", "daily", "weekly", "monthly"];
	...
	if (appCtxt.isOffline) {
        ...
        ZmPref.registerPref("AUTO_ARCHIVE_ENABLED", {
            displayName:		ZmMsg.autoArchiveDesc,
            displayContainer:	ZmPref.TYPE_CHECKBOX
        });

        ZmPref.registerPref("AUTO_ARCHIVE_AGE", {
            displayName:		ZmMsg.autoArchiveAge,
            displayContainer:	ZmPref.TYPE_INPUT
        });
        ZmPref.registerPref("AUTO_ARCHIVE_FREQ", {
            displayName:		ZmMsg.autoArchiveFreq,
            displayContainer:	ZmPref.TYPE_SELECT,
            displayOptions:		autoArchiveFreqOptions,
            options:            autoArchiveFreqValueOptions
        });

        ZmPref.registerPref("PASSWORD_LOCK_ENABLED", {
            displayName:		ZmMsg.passwordLockCheckBoxLabel,
            displayContainer:	ZmPref.TYPE_CHECKBOX
        });
	}
