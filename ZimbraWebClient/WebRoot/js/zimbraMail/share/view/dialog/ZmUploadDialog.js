...
function(folder, callback, title, loc, oneFileOnly, noResolveAction, showNotes) {
    ...
    // Fix for bug: 53347. Updates the upload dialog with active user's briefcase upload limit.
...
ZmUploadDialog.prototype._upload = function(){
    ...
    // Fix for bug: 53347.
    var activeUserId = appCtxt.getActiveAccount().id;

    if (form.action.length) {
        var uri = form.action.indexOf('?') > 0 ? '&limitAcct=' + activeUserId : 'limitAcct=' + activeUserId;
        form.action += uri;
    }
