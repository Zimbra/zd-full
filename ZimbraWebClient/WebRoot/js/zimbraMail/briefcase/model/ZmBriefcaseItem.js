...
ZmBriefcaseBaseItem.prototype.getModifyDate =
function(node) {
    if (node.d && (Number(node.md) < Number(node.d))) {
        //node.md is seconds since epoch; indicates Zimbra 8+ server response data
        var mdMilliSecs = Number(node.md) * 1000;
        return new Date(mdMilliSecs);
    } else {
        return new Date(Number(node.md));
    }
};
...
function(node) {
	...
	if (node.md)	{
        this.modifyDate = this.getModifyDate(node);
    }
...
function(data) {
	...
	if (data.md) {
        this.modifyDate = this.getModifyDate(data)
	}
