...
function(params) {
	...
	if (appCtxt.isOffline) {
		...
		params2.omit = {};
        // Fix for bug: 62973. Allow local folder to be selectable.
		params2.selectable = true;
	...
	if (hi) {
		if (updateStatus) {
            ...
            if (appCtxt.isOffline && account.status == ZmZimbraAccount.STATUS_AUTHFAIL) {
                this._handleAuthFailed(account);
            }
		}
...
function(ev) {
	...
	if (header) {
		...
		if (data == ZmOrganizer.ID_ALL_MAILBOXES) {
			var origText = AjxStringUtil.htmlEncode(header.__origText) ;
			var text = expanded
				? origText
				: this._getFolderLabel(ZmOrganizer.ID_INBOX, origText);
			header.setText(text);
...
function(acct, header) {
	...
	if (inbox && inbox.numUnread > 0) {
		var name = AjxMessageFormat.format(ZmMsg.folderUnread, [AjxStringUtil.htmlEncode(acct.getDisplayName()), inbox.numUnread]);
...
ZmAccountOverviewContainer.prototype._handleAuthFailed =
function(account) {
	if (account.code == ZmZimbraAccount.TWO_FACTOR_SETUP_REQUIRED) {
		var dialog = appCtxt.getTwoFactorCodeChangeDialog(false, ZmMsg.offlineTwoFactorSetupRequired);
	}
	else if (account.code == ZmZimbraAccount.TWO_FACTOR_AUTH_REQUIRED) {
		var dialog = appCtxt.getTwoFactorCodeChangeDialog(false, ZmMsg.offlineTwoFactorCodeRequired);
	}
    else if (account.code == ZmZimbraAccount.TWO_FACTOR_AUTH_EXPIRED) {
        var dialog = appCtxt.getTwoFactorCodeChangeDialog(false, ZmMsg.offlineTwoFactorAuthExpired);
    }
	else {
		//Close if two factor auth dialog if it is seen.
		var dialog = appCtxt.getTwoFactorCodeChangeDialog(true);
		if (dialog) {
			dialog.popdown();
		}
		//Get the password change dialog
		dialog = appCtxt.getPasswordChangeDialog();
	}
