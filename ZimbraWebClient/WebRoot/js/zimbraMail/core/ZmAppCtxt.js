...
/**
 * Gets the password lock dialog.
 *
 * @return	{ZmPasswordLockDialog}	the password lock dialog
 */
ZmAppCtxt.prototype.getPasswordLockDialog =
function() {
	if (!this._passwordLockDialog) {
		this._passwordLockDialog = new ZmPasswordLockDialog(this._shell);
	}
	return this._passwordLockDialog;
};
...
/**
 * Gets the two factor code update dialog.
 * @param {boolean} dontCreate  - don't create ZmTwoFactorCodeUpdateDialog if not created already.
 * @param  {String} The string to be used for error message
 *
 * @return  {ZmTwoFactorCodeUpdateDialog}
 */
ZmAppCtxt.prototype.getTwoFactorCodeChangeDialog =
function(dontCreate, msgString) {
    if (!this._twoFactorCodeUpdateDialog) {
        if (dontCreate) {
            return null;
        }
        AjxDispatcher.require("Extras");
        this._twoFactorCodeUpdateDialog = new ZmTwoFactorCodeUpdateDialog(this._shell);
    }
    if (msgString) {
        this._twoFactorCodeUpdateDialog.setMsgString(msgString);
    }
    return this._twoFactorCodeUpdateDialog;
};
...
/**
 * Gets the ZD's default account i.e. Local Folder account.
 *
 * @return	{ZmZimbraAccount}	the active account
 */
ZmAppCtxt.prototype.getLocalAccount =
function() {
    return this.isChildWindow
        ? parentAppCtxt.accountList.mainAccount
        : this.accountList.mainAccount;
};
