...
USAGE: $0 [-d LEVEL] (-t OR -c CHANGEID)
    ...
    -nowrite      don't write to bugzilla (for testing)
END_OF_USAGE
  die $usage;
}
...
sub do_bug_update($$) {
  ...
  ZP4::debug(2,"Changelist is:\n $cl\n---------------\n");

  my @bugIds = get_bug_ids($cl, "bug");
  my @fixedIds = get_bug_ids($cl, "fixed");
  
  # find everything in "bugIds" that ISN'T in the fixed list...
  my @notFixedBugs;
  foreach my $inBugIds (@bugIds) {
    my $found = 0;
    foreach my $inFixedBugs (@fixedIds) {
      if ($inFixedBugs == $inBugIds) {
        $found = 1;
      }
    }
    if ($found == 0) {
      push @notFixedBugs, $inBugIds;
    }
  }
  ...
  foreach my $id (@fixedIds) {
    ZP4::debug(3, "FIXED: $id");
  }
  foreach my $id (@notFixedBugs) {
    ZP4::debug(3, "NOT_FIXED: $id");
  }
  ...
  # update the NOT-FIXED bugs:
  if (@notFixedBugs > 0 && $notFixedBugs[0] ne "NONE") {
    my $fitText = fit_to_width($cl, 70);
    foreach my $id (@notFixedBugs) {
      ZP4::debug(1, "Appending(not-fixed) to BUG #$id:\n$fitText");
      if (!defined($skipWrite)) {
        eval{update_bug($id, $fitText, 0);};
      }
    }
  } 

  # update the FIXED bugs:
  if (@fixedIds > 0 && $fixedIds[0] ne "NONE") {
    my $fitText = fit_to_width($cl, 70);
    foreach my $id (@fixedIds) {
      ZP4::debug(1, "Appending(FIXED) to BUG #$id:\n$fitText");
      if (!defined($skipWrite)) {
        eval{update_bug($id, $fitText, 1);};
      }
    }
  }
...
# return an array of bugIds, or "NONE" if there is a bug:none line
#
sub get_bug_ids($$) {
  my ($message, $identifier) = @_;
  my @toRet;
