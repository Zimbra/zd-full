...
function() {
	...
	for (var i = 0; i <  allz.length; i++) {
		...
		// Bug: 83464. Restrict loading of social zimlet & its pref.
		// Bug: 96571. Remove LinkedIn zimlet from Zimbra Desktop
		if (allz[i].zimlet[0].name === "com_zimbra_social" || allz[i].zimlet[0].name === "com_zimbra_linkedin") {
			continue;
		}
...
function(html, idx, item, field, colIdx, params) {
	// Bug: 83464. Restrict loading of social zimlet & its pref.
	// Bug: 96571. Remove LinkedIn zimlet from Zimbra Desktop
	if (item.name === "com_zimbra_social" || item.name === "com_zimbra_linkedin") {
		return;
	}
