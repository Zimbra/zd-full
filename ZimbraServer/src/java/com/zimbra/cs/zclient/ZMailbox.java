...
public class ZMailbox implements ToZJSONObject {
    ...
    public static class Options {
		...
		public Options(String account, AccountBy accountBy, String password, String uri, String trustedToken) {
			this(account, accountBy, password, uri);
			this.mTrustedDeviceToken = trustedToken;
		}
        ...
        public void setTwoFactorCode(String code) { mTwoFactorCode = code;}
		...
		public void setTrustedDeviceToken(String trustedToken) { this.mTrustedDeviceToken = trustedToken; }
    }
    ...
    private ZAuthResult authByPassword(Options options, String password) throws ServiceException {
        ...
        if (options.getTwoFactorCode() != null) { // when we have a fresh totp
                                                  // always use it. Else use the
                                                  // trusted token.
            auth.setTwoFactorCode(options.getTwoFactorCode());
            // Always trust the device
            auth.setDeviceTrusted(true);
        }
        else if(options.getTrustedDeviceToken() != null) {
        ...
        if (authRes.getTwoFactorAuthRequired()) {
            if (auth.getTrustedDeviceToken() != null) { // We had sent a trusted
                                                        // token. Looks like it
                                                        // is expired.
                throw AccountServiceException.TWO_FACTOR_AUTH_EXPIRED();
            } else {
