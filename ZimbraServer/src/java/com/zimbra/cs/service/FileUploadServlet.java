...
public class FileUploadServlet extends ZimbraServlet {
    ...
    //ZD uses family mailbox, so all uploads technically go into parent account (i.e. OfflineProvisioning.getLocalAccount)
    //we expose this parameter so UI can specify which account to use when checking upload size limits
    protected static final String PARAM_LIMIT_ACCOUNT_ID = "limitAcct";
    ...
    private static Upload fetchRemoteUpload(String accountId, String uploadId, AuthToken authtoken) throws ServiceException {
        ...
        try {
            ...
            Upload up = saveUpload(get.getResponseBodyAsStream(), filename, contentType, accountId, accountId);
            synchronized (mProxiedUploadIds) {
    ...
    public static Upload saveUpload(InputStream is, String filename, String contentType, String accountId, boolean limitByFileUploadMaxSize) throws ServiceException, IOException {
        return saveUpload(is, filename, contentType, accountId, getFileUploadMaxSize(limitByFileUploadMaxSize, accountId));
    }

    public static Upload saveUpload(InputStream is, String filename, String contentType, String accountId, boolean limitByFileUploadMaxSize, String uploadLimitAccountId) throws ServiceException, IOException {
        return saveUpload(is, filename, contentType, accountId, getFileUploadMaxSize(limitByFileUploadMaxSize, uploadLimitAccountId));
    }
    ...
    public static Upload saveUpload(InputStream is, String filename, String contentType, String accountId) throws ServiceException, IOException {
        return saveUpload(is, filename, contentType, accountId, false, accountId);
    }

    public static Upload saveUpload(InputStream is, String filename, String contentType, String accountId, String uploadLimitAccountId) throws ServiceException, IOException {
        return saveUpload(is, filename, contentType, accountId, false, uploadLimitAccountId);
    }
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        try {
            ...
            String uploadLimitAccountId = req.getParameter(PARAM_LIMIT_ACCOUNT_ID);
            // file upload requires multipart enctype
            if (ServletFileUpload.isMultipartContent(req)) {
                handleMultipartUpload(req, resp, fmt, at.getAccountId(), limitByFileUploadMaxSize, uploadLimitAccountId);
            } else {
                handlePlainUpload(req, resp, fmt, at.getAccountId(), limitByFileUploadMaxSize, uploadLimitAccountId);
            }
        } catch (ServiceException e) {
    ...
    List<Upload> handleMultipartUpload(HttpServletRequest req, HttpServletResponse resp, String fmt, String accountId, boolean limitByFileUploadMaxSize, String uploadLimitAccountId)
    throws IOException, ServiceException {
        List<FileItem> items = null;
        String reqId = null;

        ServletFileUpload upload = getUploader(limitByFileUploadMaxSize, uploadLimitAccountId);
        try {
    ...
    List<Upload> handlePlainUpload(HttpServletRequest req, HttpServletResponse resp, String fmt, String accountId, boolean limitByFileUploadMaxSize, String uploadLimitAccountId)
    throws IOException, ServiceException {
        // metadata is encoded in the response's HTTP headers
    ...
    private static long getFileUploadMaxSize(boolean limitByFileUploadMaxSize, String uploadLimitAccountId) throws ServiceException {
        // look up the maximum file size for uploads
        long maxSize = DEFAULT_MAX_SIZE;
        if (!StringUtil.isNullOrEmpty(uploadLimitAccountId)) {
            Account account = Provisioning.getInstance().getAccount(uploadLimitAccountId);
            if (account == null) {
                throw ServiceException.INVALID_REQUEST("unknown accountId [" + uploadLimitAccountId + "]", null);
            }
            if (limitByFileUploadMaxSize) {
                maxSize = account.getLongAttr(Provisioning.A_zimbraFileUploadMaxSize, DEFAULT_MAX_SIZE);
            } else {
                maxSize = account.getLongAttr(Provisioning.A_zimbraMtaMaxMessageSize, DEFAULT_MAX_SIZE);
            }
        } else {
            if (limitByFileUploadMaxSize) {
    ...
    protected ServletFileUpload getUploader(boolean limitByFileUploadMaxSize, String uploadLimitAccountId) throws ServiceException {
    	return getUploader(getFileUploadMaxSize(limitByFileUploadMaxSize, uploadLimitAccountId));
    }
