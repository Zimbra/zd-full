...
public class ZcsMailbox extends ChangeTrackingMailbox {
    ...
    public ZAuthToken getAuthToken() throws ServiceException {
        ...
        if (authToken == null) {
            ...
            // We add the trusted token here.
            final String trustToken = getAccount().getAttr(ZAttrProvisioning.A_zimbraTwoFactorAuthTrustedDeviceTokenKey);
            if (trustToken != null) {
                request.addElement(AccountConstants.E_TRUSTED_TOKEN).setText(trustToken);
            }
            ...
            if (response != null) {
                if (response.getAttributeBool(AccountConstants.E_TWO_FACTOR_AUTH_REQUIRED, false)) {
                    if (trustToken != null) { // We had sent a trusted token. Looks like it is expired.
