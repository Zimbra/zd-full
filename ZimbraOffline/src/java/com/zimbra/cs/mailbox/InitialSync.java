...
public class InitialSync {
    ...
    static List<Element> fetchContacts(ZcsMailbox ombx, String ids) throws ServiceException {
        try {
            ...
            if (ombx.getRemoteServerVersion().getMajor() >= 8)
                request.addAttribute(MailConstants.A_DEREF_CONTACT_GROUP_MEMBER, true);
    ...
    void syncContact(Element elt, int folderId) throws ServiceException {
        ...
        for (Element eField : elt.listElements()) {
            //we should deal with metadata sync when there's api to set them
            ...
            while ((te = tis.getNextEntry()) != null) {
                ...
                try {
                    ...
                    if (ombx.getItemRevision(sContext, existingItem.getId(), MailItem.TYPE_UNKNOWN, doc.getVersion()) == null ||
                            (!StringUtil.equal(existingItem.getName(), doc.getName()))) {
                ...
                if (flags != 0 || getTagSync().isMappingRequired()) {
