...
string VirtualMachine::GetRegistryKey(string keyPath, string key) {
    ...
    LONG retValue = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyPath.c_str(), 0, KEY_READ|KEY_WOW64_64KEY, &hKey) ;
...
bool CanOpenRegistry(string registry) {
    HKEY hKey = 0;
    //Open registry path where key is stored
    LONG retValue = RegOpenKeyEx(HKEY_LOCAL_MACHINE, registry.c_str(), 0, KEY_READ|KEY_WOW64_64KEY, &hKey) ;
...
string VirtualMachine::FindJava() {
    ...
    if (IsJREInstalled()) {
        ...
        Log("64 bit JRE is not found on your system.");
    }
    return "";
}
...
DWORD WINAPI VirtualMachine::JvmThreadMain(LPVOID lpParam) {
    ...
    if (jvmPath.empty()) {
        self->last_err = "Zimbra Desktop (64-bit) requires a 64-bit Java SE Runtime Environment (JRE), which is not installed or is outdated. Please install JRE 1.6 (64-bit) or later.";
        goto error0;
    }
