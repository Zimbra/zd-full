...
public class OfflineImport implements DataImport {
    ...
    public static OfflineImport imapImport(OfflineDataSource ds)
        ...
        return new OfflineImport(ds, new ImapSync(ds),
            "imap", IMAP_INTERVAL);
    }
    ...
    public static OfflineImport ycalImport(OfflineDataSource ds)
        throws ServiceException {
        //Check for yahoo service type and then hit appropriate Yahoo! CalDav server.
        String yahooService = ds.getEmailAddress().endsWith("yahoo.co.jp") ? "yahoo.co.jp" : "yahoo.com";
        return new OfflineImport(
            ds, new OfflineCalDavDataImport(ds, yahooService), "caldav", CALENDAR_INTERVAL);
    }
