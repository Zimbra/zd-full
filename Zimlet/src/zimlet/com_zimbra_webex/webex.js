...
/**
 * Maintain list of ZCS account list and if any zimlet meta data found
 */
WebExZimlet.ZCS_ACCOUNT_LIST = [];
WebExZimlet.ZCS_ACC_META_DATA = false;
WebExZimlet.ZD_ACC_META_DATA = false;
/**
 * Initializes the zimlet.
 *
 */
WebExZimlet.prototype.init = function() {
    this.metaData = appCtxt.getLocalAccount().metaData;
	//this._displayGeneralPrefsDlg();
    this._getZCSAccountList();
    this._fetchZimletMetaDataFromLocalAccount();
    this.zcsMetaData = null;
...
function(postCallback) {
    var accountName = appCtxt.getLocalAccount().name;

    // If ZCS_ACC_META_DATA is true that means there was webex zimlet metadata configured on ZCS account earlier.
    if (WebExZimlet.ZCS_ACC_META_DATA === true && WebExZimlet.ZD_ACC_META_DATA === false) {
        this.zcsMetaData = WebExZimlet.ZCS_ACCOUNT_LIST[0].metaData;
        this.zcsMetaData.get("webexZimletAccountPreferences", new ZmBatchCommand(null, WebExZimlet.ZCS_ACCOUNT_LIST[0].name, null), new AjxCallback(this, this._handleGetAccPrefsMetaData, postCallback));
    }
    else {
        this.metaData.get("webexZimletAccountPreferences", new ZmBatchCommand(null, accountName, null), new AjxCallback(this, this._handleGetAccPrefsMetaData, postCallback));
    }
};
...
WebExZimlet.prototype._fetchZimletMetaDataFromLocalAccount =
function(result) {
    if (result) {
        var webexPrefs = result.getResponse().BatchResponse.GetMailboxMetadataResponse[0].meta[0]._attrs;
        if (webexPrefs) {
            WebExZimlet.ZD_ACC_META_DATA = true;
            return;
        }
        else {
            WebExZimlet.ZD_ACC_META_DATA = false;
            this._fetchMetaDataFromZCSAccount();
            return;
        }
    }

    var accountName = appCtxt.getLocalAccount().name;
    this.metaData.get("webexZimletAccountPreferences", new ZmBatchCommand(null, accountName, null), new AjxCallback(this, this._fetchZimletMetaDataFromLocalAccount));
};

WebExZimlet.prototype._fetchMetaDataFromZCSAccount =
function(result) {
    if (result) {
        var webexPrefs = result.getResponse().BatchResponse.GetMailboxMetadataResponse[0].meta[0]._attrs;
        if (webexPrefs) {
            WebExZimlet.ZCS_ACC_META_DATA = true;
            return;
        }
        else {
            WebExZimlet.ZCS_ACC_META_DATA = false;
    ...
    this.zcsMetaData = WebExZimlet.ZCS_ACCOUNT_LIST.length && WebExZimlet.ZCS_ACCOUNT_LIST[0].metaData;
    this.zcsMetaData.get("webexZimletAccountPreferences", null, new AjxCallback(this, this._fetchMetaDataFromZCSAccount));
};

WebExZimlet.prototype._getZCSAccountList =
function() {
    var zimbraAccountList = appCtxt.accountList.visibleAccounts.filter(function(account) {
        if (account.type === ZmAccount.TYPE_ZIMBRA && !account.isMain) {
            return account;
        }
    });

    WebExZimlet.ZCS_ACCOUNT_LIST = zimbraAccountList;
};
...
function() {
    ...
    // Set the webex details to local account. This way the zimlet metadata is now migrated from ZCS account to ZD local account.
    this.metaData = appCtxt.getLocalAccount().metaData;
...
function() {
    ...
    // Do not lookup meta data in ZCS account since we have already saved it to ZD's local account meta data
    WebExZimlet.ZCS_ACC_META_DATA = false;
};
...
function(postCallback) {
    ...
    if (WebExZimlet.ZCS_ACC_META_DATA === true && WebExZimlet.ZD_ACC_META_DATA === false) {
        this.zcsMetaData = WebExZimlet.ZCS_ACCOUNT_LIST[0].metaData;
        this.zcsMetaData.get("webexZimletGeneralPreferences", new ZmBatchCommand(null, WebExZimlet.ZCS_ACCOUNT_LIST[0].name, null), new AjxCallback(this, this._handleGetGeneralPrefsMetaData, postCallback));
    }
    else {
        this.metaData.get("webexZimletGeneralPreferences", new ZmBatchCommand(null, accountName, null), new AjxCallback(this, this._handleGetGeneralPrefsMetaData, postCallback));
    }
};
